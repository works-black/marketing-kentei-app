      ...
      <div className="space-y-3">
        {question.choices.map((choice, index) => (
          <button
            ...
            disabled={answered}
            className={`w-full flex items-start gap-3 p-4 rounded-xl border-2 text-left transition-all ${choiceStyle(index)}`}
          >
            <span
              className={`shrink-0 w-7 h-7 rounded-full flex items-center justify-center text-sm font-bold transition-colors ${labelStyle(index)}`}
            >
              {LABELS[index]}
            </span>
